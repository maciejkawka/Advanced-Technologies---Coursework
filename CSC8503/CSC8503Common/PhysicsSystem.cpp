...
void PhysicsSystem::Update(float dt) {	
	...
	if (Window::GetKeyboard()->KeyPressed(KeyboardKeys::N)) {
		//springReloution = !springReloution;
		//std::cout << "Setting SpringRelotution to " << springReloution << std::endl;
		resolveType++;
		if (resolveType >= 3)
			resolveType = 0;
		if(resolveType == 0)
			std::cout << "Setting Penalty Mode" << std::endl;
		else if(resolveType == 1)
			std::cout << "Setting Impulse Mode" << std::endl;
		else if (resolveType == 2)
			std::cout << "Setting Projection Mode" << std::endl;
	}
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	Vector3 normalImpulse = p.normal * j;

	//Fraction calsulation - if dynamicFracion==0 use simple fracion calsulation
	Vector3 frisctionImpulse = Vector3();
	if (physA->GetStaticFriction() != 0 && physB->GetStaticFriction() != 0)
	{
		if (physA->GetDynamicFriction() != 0 || physB->GetDynamicFriction() !=0)
		{
			//Attempt to implement dynamic and static friction
			Vector3 t = contactVelocity - (p.normal * Vector3::Dot(contactVelocity, p.normal));
			float frictionAngularEffect = Vector3::Dot(inertiaA + inertiaB, t.Normalised());
			float jt = (-Vector3::Dot(contactVelocity, t.Normalised())) / (totalMass + frictionAngularEffect);

			float friction = (physA->GetStaticFriction() + physA->GetStaticFriction()) / 2;

			if (abs(jt) < j * friction)
				frisctionImpulse = t.Normalised() * jt;
			else
			{
				friction = (physA->GetDynamicFriction() + physB->GetDynamicFriction()) / 2;
				frisctionImpulse = -t.Normalised() * j * friction;
			}
		}
		else
		{
			float friction = (physA->GetStaticFriction() + physB->GetStaticFriction()) / 2;
			Vector3 t = contactVelocity - (p.normal * Vector3::Dot(contactVelocity, p.normal));
			float frictionAngularEffect = Vector3::Dot(inertiaA + inertiaB, t.Normalised());
			float jt = (-friction * Vector3::Dot(contactVelocity, t.Normalised())) / (totalMass + frictionAngularEffect);
			frisctionImpulse = t.Normalised() * jt;
		}
	}
	
	physA->ApplyLinearImpulse(-normalImpulse - frisctionImpulse);
	physB->ApplyLinearImpulse(normalImpulse + frisctionImpulse);

	physA->ApplyAngularImpulse(Vector3::Cross(relativeA,(-normalImpulse - frisctionImpulse)));
	physB->ApplyAngularImpulse(Vector3::Cross(relativeB, normalImpulse + frisctionImpulse));
...
void PhysicsSystem::ResolveSpringCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const
{
	...
	float springConst = (physA->GetElasticity() + physB->GetElasticity())/2;
	float force = -springConst * p.penetration;
	Vector3 fullForce = p.normal * force;


	Vector3 angForceA = Vector3::Cross(relativeA, fullForce);
	Vector3 angForceB = Vector3::Cross(relativeB, -fullForce);

	physA->ApplyLinearImpulse(fullForce);
	physB->ApplyLinearImpulse(-fullForce);

	physA->ApplyAngularImpulse(angForceA);
	physB->ApplyAngularImpulse(angForceB);
}

void NCL::CSC8503::PhysicsSystem::ProjectionResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const
{
	PhysicsObject* physA = a.GetPhysicsObject();
	PhysicsObject* physB = b.GetPhysicsObject();
...
void PhysicsSystem::IntegrateAccel(float dt) {
	...
	for (auto i = first; i != last; ++i)
	{
		...
		if ((!object->isKinematic()) && applyGravity && inverseMass > 0 && object->GetGravity())
			accel += gravity;
