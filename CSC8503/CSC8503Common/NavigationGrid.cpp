...
bool NavigationGrid::FindPath(const Vector3& from, const Vector3& to, NavigationPath& outPath) {
	...
	//std::vector<GridNode*>  openList; //from tutorial
	std::set<GridNode*>  openList; //Decided to use Set find operation is cheaper in set, and it always keep the container in defined order
	std::set<GridNode*>  closedList; //Decided to use Set find operation is cheaper in set, and it always keep the container in defined order
	...
	while (!openList.empty()) {
		//currentBestNode = RemoveBestNode(openList); //from tutoral
...
bool NavigationGrid::NodeInList(GridNode* n, std::set<GridNode*>& list) const {
	std::set<GridNode*>::iterator i = std::find(list.begin(), list.end(), n);
	return i == list.end() ? false : true;
}
