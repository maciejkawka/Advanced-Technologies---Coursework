...
namespace NCL {
	...
	namespace CSC8503 {
		...
		protected:
			...
			void OperateOnIntersectionContents(QuadTreeFunc& func, Ray& r)
			{
				if (children)
				{
					for (int i = 0; i < 4; ++i)
					{
						...
						Vector2 childSize = children[i].size;
		
						GameObject nodeObject;
						AABBVolume* volume = new AABBVolume(Vector3(childSize.x,50.0f,childSize.y));
						nodeObject.SetBoundingVolume((CollisionVolume*)volume);
						nodeObject.GetTransform().SetPosition(Vector3(childPos.x, 0, childPos.y));
						

						RayCollision thisCollision;
						if (!CollisionDetection::RayIntersection(r, nodeObject, thisCollision))
							continue;

						children[i].OperateOnIntersectionContents(func, r);
					}
				}
				else
				{
...
namespace NCL {
	...
	namespace CSC8503 {
		...
		public:
			...
			void OperateOnIntersectionContents(typename QuadTreeNode<T>::QuadTreeFunc  func, Ray& r) {
				root.OperateOnIntersectionContents(func, r);
			}
